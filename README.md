...
- 支持从 BOSS、猎聘推荐岗位中按自动消息配置筛选，并调用平台默认招呼语批量发起沟通
- 自动消息支持使用用户自己的 DeepSeek API Key，将简历、期待岗位和补充要求作为发送前最后一道 AI 匹配条件
...
开发版结果页标题右侧显示 `log` 和 `debug` 两个开关，默认均开启，也可以通过 URL 参数覆盖，例如 `log=false&debug=false`。打包版不显示开关且默认关闭，但仍可通过 `log=true` 或 `debug=true` 临时启用；自动消息面板可在正式版按住 `Control` 点击“自动消息”临时开启日志。`debug` 用于控制总览页内部数据导入导出。
...
自动消息面板可以展开“AI匹配”配置。AI 匹配开启后，扩展会先完成薪资、年限、在线状态、
关键词、排除条件和重复沟通检查，只对全部通过的岗位调用 DeepSeek；AI 判断匹配后才正式占位
并发送招呼。DeepSeek API Key、简历和提示词保存在浏览器本地，不经过项目服务器；简历和精简
岗位信息会由扩展直接发送给 DeepSeek。详细设计见[自动消息 AI 匹配修改方案](docs/ai_match.md)。
开启自动消息调试日志后，会额外记录 DeepSeek 的完整请求体和响应体以便排查；其中可能包含简历、
提示词和岗位描述，API Key 与 Authorization Token 始终隐藏。
...
后二次判断。猎聘的技术关键字、职位关键字和岗位关键字过滤器统一只检索详情页
`job-intro-content` 中的职位介绍正文。详细链路见[猎聘推荐岗位批量自动打招呼](docs/liepin-auto-greeting-plan.md)。

BOSS 的对应四项过滤也在详情请求前完成，分别只读取推荐列表的 `salaryDesc`、
`jobExperience`、`bossOnline` 和 `brandName`；岗位详情仅用于仍依赖正文的关键词过滤与记录补全。
自动消息配置中的“非猎头”启用后，BOSS 会跳过推荐列表中 `goldHunter=1` 的岗位，猎聘会
跳过 `job.jobKind=1` 的岗位，两者都在请求岗位详情前完成。
扩展弹窗中的“仅在线”和“非猎头”是网页岗位筛选，均按当前标签页保存，修改后需刷新招聘
页面生效；网页“非猎头”会按同一字段隐藏猎头岗位。自动消息面板中的“非猎头”只控制
自动消息候选岗位，与弹窗网页筛选互不影响。

在日志模式中，猎聘自动消息会输出期望、推荐、岗位详情和默认招呼语接口的完整请求与响应；
浏览器自动携带的 Cookie 以及 XSRF Token 会隐藏，避免在日志中暴露登录凭据。
...
3. 可打开“发送消息”面板配置并启动推荐岗位自动打招呼
4. 在 BOSS 或猎聘推荐岗位页可勾选“仅在线”“非猎头”，或勾选“不显示”并用 `|` 分隔不想看到的公司关键字，刷新页面后过滤对应岗位
5. 点击“同步当前聊天记录”
6. 在同步结果页确认并保存记录
7. 在总览页筛选、搜索、编辑、导出记录；选中同一网站的记录后可批量发送文本消息
