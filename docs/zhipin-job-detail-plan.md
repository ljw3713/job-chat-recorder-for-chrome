...
错误文本仍需截断；当前结果页的一次性请求日志则按下述规则显示完整响应。
...
日志通过运行时消息发送到当前结果页并仅保存在页面内存中；不写入
`chrome.storage.local`，关闭或刷新结果页后即丢失。

开发工作区通过 `runtime-config.js` 默认开启 `log` 和 `debug`，并在结果页标题右侧显示对应复选框。URL 参数 `log=true|false` 和 `debug=true|false` 可覆盖默认值。打包脚本会将发布包的 `enableDebugLog` 固定替换为 `false`，因此发布版本默认关闭且隐藏复选框，但仍可通过 URL 参数临时启用。
...
总览页使用 `debug=true` 时，CSV 增加“内部数据”列并保留岗位相关字段。

调试 CSV 导入同一 `recordKey` 时：

- 导入的内部岗位字段覆盖数据库中的同名字段。
- 深度合并 `boss`、`lastMessageInfo` 和 `liepin`。
- 没有内部数据时仍保留已有发送、同步和岗位字段。
...
- 开发工作区默认使用 `log=true&debug=true`，并显示两个复选框。
- 发布包默认关闭并隐藏复选框，但保留 URL 参数控制能力。
...
1. 在总览页使用 `debug=true` 导出一次完整 CSV 备份。
2. 确认调试 CSV 存在“内部数据”列。
3. 上线重构版本。
4. 清除旧岗位相关数据，或清空记录后导入 CSV。
5. 通过同步页重新补齐新增及缺失岗位信息。
6. 在总览页对指定记录执行全量岗位刷新。

普通 CSV 没有“内部数据”列，只能恢复可见字段，不能完整恢复发送和同步内部状态。
