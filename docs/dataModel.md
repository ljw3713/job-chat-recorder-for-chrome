...
记录已经存在显式 `recordKey` 时，该值是权威且稳定的唯一标识。例外是 BOSS 自动打招呼
记录：全量聊天同步以相同的“加密招聘者 ID + `jobRef.externalId`”命中后，会将其以
`externalId` 生成的旧键迁移为当前聊天列表的数字 `boss.jobId` 键，以避免重复记录。仅缺少
`recordKey` 的旧记录才根据下述规则补齐。
- 删除、忽略、发送和更新岗位详情。
- 判断记录是否已存在。
