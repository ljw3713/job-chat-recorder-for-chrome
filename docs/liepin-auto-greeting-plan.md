...
本文同时作为当前实现说明，已包含推荐接口的分页拉取；其余已确认链路已按本文落地。
...
2. 按用户选择的求职期望获取首页推荐岗位，并在存在下一页时继续拉取。
...
首个请求固定发送 `operateKind=LOGIN`。每次响应都读取 `data.data` 并追加到候选列表；若
`data.hasNextPage === true`，立即以相同请求体再次请求，但将 `operateKind` 改为 `UP`。
`sortType`、序列化后的 `selectedExpect` 和 `existFallbackResult` 在所有页保持不变。响应
`hasNextPage` 为非 `true` 时停止拉取。

所有页合并后才进入现有候选处理循环；该循环以 `recruiterId + jobId` 去重，并继续执行既有
筛选和成功数量上限。候选耗尽时仍以“推荐岗位已处理完毕”正常结束，即使成功数未达到目标。
...
1. `open-chat` 对“已沟通过”“达到次数限制”“操作频繁”“账号受限”的响应样本。
2. `open-chat` HTTP 成功但 `flag !== 1` 时各业务码是否明确未发送。
3. 默认招呼语是否始终由服务端自动写入，还是受账号设置或岗位类型影响。
4. 同一招聘者不同岗位的 `chatted` 状态是否始终一致。
5. `ck_id` 在其他入口或分页候选中是否仍为空。
6. `shieldComp=true` 的业务含义及是否需要根据岗位类型变化。
7. 是否需要把成功后的 `chat-list` 作为首期必选步骤，以保存真实消息正文。
