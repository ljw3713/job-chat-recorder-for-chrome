...
扩展弹窗在“查看记录总览”按钮下方增加“仅在线”“非猎头”复选框。设置按当前标签页保存，
修改后需要刷新招聘页面才能生效，悬停文字时立即显示自定义刷新提示。
...
2. 悬停“仅在线”或“非猎头”文字时，通过 `data-tooltip` 和 CSS `:hover` 立即显示“修改后需要刷新当前招聘页面才能生效”。
...
只缓存提取后的离线/猎头岗位 `encryptJobId` 或岗位链接，不缓存完整响应。
...
- “仅在线”“非猎头”和“不显示”为或关系，岗位命中任意一个启用的过滤条件就会被过滤。
